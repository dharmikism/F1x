...
const api = async (path, options = {}) => {
  ...
  if (!response.ok) throw new Error(body.detail || "The FixOnce service is unavailable.");
  return body;
};
const escapeHtml = (value = "") => String(value).replace(/[&<>'"]/g, (char) => ({ "&":"&amp;", "<":"&lt;", ">":"&gt;", "'":"&#39;", '"':"&quot;" }[char]));
const toast = (message, error = false) => {
  const item = document.createElement("div"); item.className = `toast${error ? " error" : ""}`; item.textContent = message; $("#toast-region").appendChild(item); setTimeout(() => item.remove(), 3500);
};
...
function renderKnowledge(items = state.knowledge) {
  ...
  $("#knowledge-list").innerHTML = filtered.slice(0, 7).map(item => `
    <div class="knowledge-row">
      ...
      <div class="knowledge-cell"><strong>${item.verification_count}</strong> verifications</div>
      <div class="knowledge-cell"><strong>${item.usage_count}</strong> reuses</div><div class="knowledge-arrow">↗</div>
    </div>`).join("");
}
...
function renderNew(result, blocked = false) {
  const title = blocked ? "Kept out of community memory" : "We couldn't find a known fix";
  const status = blocked ? "PRIVATE / TIME-SENSITIVE" : "NEW PROBLEM";
  $("#result-region").innerHTML = `<div class="result-card new-result ${blocked ? "blocked-card" : ""}">
    <div class="result-main"><div class="result-status"><span class="status-check" style="background:${blocked ? "var(--warm)" : "var(--purple)"};color:#201d2d">${blocked ? "!" : "+"}</span> ${status}</div><h3 class="result-title">${title}</h3><p class="result-problem">${escapeHtml(result.problem)}</p><div class="solution-box">${solutionText(result.suggestion)}</div><div class="result-meta"><span class="meta-pill">${escapeHtml(result.provider)}</span><span class="meta-pill">${result.latency_ms} ms total</span></div>${blocked ? `<div class="share-box" style="background:#33271e;border-color:#604c36"><h4>Privacy guardrail</h4><p>${escapeHtml(result.safety.reason || "This result is not eligible for reuse.")}</p></div>` : `<div class="share-box" id="verify-box"><h4>Did this solve the problem?</h4><p>Test the suggested fix first. Only verified solutions can be shared, and sharing is always optional.</p><div class="share-actions"><button class="secondary-button" data-verify="true" data-id="${result.draft_id}">✓ Yes, it worked</button><button class="ghost-button" data-verify="false" data-id="${result.draft_id}">No, keep private</button></div></div>`}</div>
    <aside class="result-aside"><h3>${blocked ? "Reuse bypassed" : "Suggested fix"}</h3><p>${blocked ? "FixOnce avoids reusing or publishing personal and time-sensitive requests." : "A fresh suggestion is generated only after community memory comes up empty."}</p><div class="aside-fact"><span>Search time</span><strong>${result.search_latency_ms} ms</strong></div><div class="aside-fact"><span>AI time</span><strong>${result.ai_latency_ms} ms</strong></div><div class="aside-fact"><span>Saved by default</span><strong>NO</strong></div></aside></div>`;
}

function renderVerified(id) {
  const box = $("#verify-box"); if (!box) return;
  box.className = "share-box verified-card";
  box.innerHTML = `<h4>✓ Solution verified</h4><p>This fix worked for you. Share it so the next person can skip the rediscovery step.</p><div class="share-actions"><button class="secondary-button" data-share="true" data-id="${id}">Share with community</button><button class="ghost-button" data-share="false" data-id="${id}">Keep private</button></div>`;
}

async function handleLookup(event) {
  event.preventDefault();
  const input = $("#problem-input"); const problem = input.value.trim();
  if (!problem) { toast("Describe the problem first.", true); input.focus(); return; }
  const button = $("#find-button"); button.disabled = true; button.innerHTML = `<span class="button-icon">◌</span> Searching community memory…`;
  $("#result-region").innerHTML = `<div class="result-card new-result"><div class="result-main"><div class="result-status"><span class="status-check" style="background:var(--purple);color:#201d2d">⌕</span> SEARCHING COMMUNITY MEMORY</div><h3 class="result-title">Checking solved problems for a semantic match…</h3><p class="result-problem">The words can change. The underlying problem is what matters.</p></div></div>`;
  try { const result = await api("/api/find-fix", { method:"POST", body:JSON.stringify({problem}) }); state.current = result; result.result_type === "known" ? renderKnown(result) : renderNew(result, result.result_type === "blocked"); await refresh(); }
...
async function handleResultClick(event) {
  ...
  try {
    if (target.dataset.feedback) { await api(`/api/knowledge/${id}/feedback`, {method:"POST", body:JSON.stringify({helpful:target.dataset.feedback === "true"})}); toast(target.dataset.feedback === "true" ? "Thanks — the community signal is stronger." : "Feedback saved."); target.disabled = true; }
